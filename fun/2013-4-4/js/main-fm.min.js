...
(function ($) {
    function mainFm(selector, params) {
        ...
        self.ff = -1;
        self.galHidHig = 60;
        if (parseInt($.browser.version, 10) == 7) $("body").css({
            "overflow-y": "hidden"
        });
        ...
        self.txtAniSpd = 600;
        self.txtAniDly = 200;
        self.fsAutoPlay = defaults.galleryAutoplay == "true" || defaults.galleryAutoplay == true;
        self.fsDelay = defaults.gallerySlideshowDelay;
        self.fsTxtSho = false;
        self.isFsGal = false;
        self.bdy.find(".fs_thumbs").each(function () {
            $(this).children().each(function () {
                var thu = $(this);
                thu.bind("mouseover mouseup mouseleave",
        ...
        self.gNex.click(function () {
            if (self.fsThuHig * (self.fpp + 1) < self.fsGal.children(":last-child").position().top + self.fsThuHig) {
                self.fpp++;
                if (Modernizr_.csstransitions) self.fsGal.css({
                    "top": -(self.fsThuHig * self.fpp) + "px"
                });
                else self.fsGal.animate({
                    "top": -(self.fsThuHig * self.fpp) + "px"
                }, 500, "easeInOutQuart")
            }
        });
        ...
        self.shGal.bind("mouseover mouseup mouseleave",
        ...
        self.shGal.click(function () {
            var sel = $(this);
            self.fpp = 0;
            if (self.fsView) self.fsGallShow(true);
            else self.fsGallHide(true)
        });
        ...
        self.fsPre.bind("mouseover mouseup mouseleave",
    ...
    mainFm.prototype = {
        initialize: function () {
            ...
            self.loading.css({
                "left": $(window).width() / 2 - self.loading.width() / 2,
            ...
            $(".headerContent li").each(function () {
                ...
                slf.bind("mouseout", function () {
                    ...
                    if (self.url != gg[1]) $("a", $(this)).css("color", self.menuColor)
                })
            });
            var ll = self.pageBackground.length;
            if (self.menuList.length > self.pageBackground.length) for (var jj = ll; jj < self.menuList.length; jj++) self.pageBackground[jj] = {
                src: ""
            };
            ...
            mNav.bind("click", function () {
                if ($(".header .headerContent .nav ul").css("display") == "none") $(".header .headerContent .nav ul").css("display", "block");
                else $(".header .headerContent .nav ul").css("display", "none")
            });
            ...
            if (self.IE_old) self.pHol.addClass("pageBackground");
            if (self.backgroundOverlay != "" && !self.mobileDevice) $.vegas("overlay", {
                src: self.backgroundOverlay
            });
            if (self.mobileDevice) for (var ii = 0; ii < self.pageBackground.length; ii++) if (self.pageBackground[ii].src_small != "" && self.pageBackground[ii].src_small) self.pageBackground[ii].src = self.pageBackground[ii].src_small;
            clearInterval(self.intr);
            $(window).resize(function () {
                clearInterval(self.intr);
                self.intr = setInterval(function () {
                    clearInterval(self.intr);
                    self.windowRez()
                }, 100)
            });
        ...
        fsSlideshow: function (ev) {
            ...
            if (ev == "delay") self.fsDly = setInterval(function () {
                clearInterval(self.fsDly);
                self.fsSlideshow("play")
            }, 500);
            if (ev == "play") self.fsInt = setInterval(function () {
                if (self.fsAutoPlay) {
                    self.fsCur = self.fsCur + 1 < self.fsArr.length ? self.fsCur + 1 : 0;
                    self.fsImgLoad(self.fsArr[self.fsCur])
                }
        ...
        fsTextOut: function () {
            ...
            if (!self.mobileDevice) {
                ...
                self.txtAniInt = setInterval(function () {
                    if (self.txtEndAni) {
                        clearInterval(self.txtAniInt);
                        if (self.fsTxt.children().length > 0) if ($(self.fsTxt.data("con")).data("num") != self.fsNum || !self.fsTxtSho) self.fsTxt.find(".fs_caption").each(function () {
                            var ll = self.fsTxt.find(".fs_caption").children().length;
                            if (!isNaN($(this).data("lt"))) var css1 = {
                                "left": $(this).data("lt") - 70,
                                "opacity": 0
                            };
                            else css1 = {
                                "right": $(this).data("rt") - 70,
                                "opacity": 0
                            };
                            ...
                            self.fsTxt.find(".fs_caption div").each(function () {
                                ...
                                $(this).delay(Number(ii * 170)).animate(css1, 250, "easeOutSine", function () {
                                    if ($(this).data("pos") == ll - 1) {
                                        ...
                                        if (self.fsTxtSho) self.fsTextIn();
                                        else self.txtEndAni = true;
                                        if (parseInt(self.pHol.css("left")) < 0) self.bg_setup()
                                    }
                                })
                            })
                        });
        ...
        fsTextIn: function () {
            ...
            var tt = false;
            var ii = 0;
            if (self.fsArr.length > 0) self.fsArr[self.fsNum].find(".fs_caption").each(function () {
                tt = true
            });
            if (tt) {
                self.fsArr[self.fsNum].find(".fs_caption").css({
                    "visibility": "visible"
                });
                ...
                self.fsTxt.find(".fs_caption").each(function () {
                    if (!$(this).data("lt")) {
                        ...
                        $(this).data({
                            "rt": parseInt($(this).css("right"))
                        })
                    }
                    $(this).show();
                    if (!isNaN($(this).data("lt"))) {
                        var css1 = {
                            "left": $(this).data("lt") + 150,
                            "opacity": 0
                        };
                        var css2 = {
                            "left": $(this).data("lt"),
                    ...
                    self.fsTxt.find(".fs_caption div").each(function () {
                        ...
                        $(this).delay(Number(ii * self.txtAniDly)).animate(css2, self.txtAniSpd, "easeOutSine", function () {
                            if ($(this).data("pos") == ll) self.txtEndAni = true
                        })
                    })
                })
            } else self.txtEndAni = true
        ...
        fsImgLoadTest: function () {
            ...
            self.fsImgInt = setInterval(function () {
                if (self.fmImgLoaded) {
                    ...
                    } else self.fsImgLoad(self.fsArr[self.fsCur])
                }
            }, 500)
        },
        fsImgLoad: function (ob) {
            var self = this;
            if (self.fsArr.length < 1 || self.fsCur < 0) return 0;
            for (var ii = 0; ii < self.fsArr.length; ii++) if (ii != ob.data("num") && self.fsArr[ii].css("opacity") < 0.99) self.fsArr[ii].css({
                "opacity": "1"
            });
            self.fsArr[ob.data("num")].css({
                "opacity": ".25"
            });
            self.fsCur = ob.data("num");
            if (self.fsView) {
                if (Modernizr_.csstransitions) self.fsGal.css({
                    "top": -self.fsArr[self.fsCur].position().top + "px"
                });
                else self.fsGal.animate({
                    "top": -self.fsArr[self.fsCur].position().top + "px"
                }, 500, "easeInOutQuart");
        ...
        fsGallShow: function (ani) {
            ...
            if (!self.mobile) {
                ...
                if (ani) {
                    ...
                    self.fsWra.animate({
                        "height": self.fsWra.children(":first-child").children(":last-child").position().top + self.fsThuHig + 15
                    }, 500, "easeInOutQuart", function () {
                        self.fsGalThmSet(true)
                    })
                } else {
        ...
        fsGallHide: function (ani) {
            ...
            if (ani && !self.mobile) setTimeout(function () {
                if (self.fsCur < 0 && self.isFsGal && parseInt(self.pHol.css("left")) == 0) {
                    self.fsCur = 0;
                    self.fsImgLoad(self.fsArr[0])
                }
            }, 500);
            else if (self.fsCur < 0 && parseInt(self.pHol.css("left")) == 0) {
                self.fsCur = 0;
                self.fsImgLoad(self.fsArr[0])
            }
            if (!self.mobile) {
        ...
        fsGalThmSet: function (view) {
            ...
            if (view) {
                sel.children(":first-child").children(":first-child").css({
                    "right": "-40px"
                });
                sel.css({
                    "width": "30px"
                ...
                if (!self.mobile) {
                    ...
                    self.pHol.css({
                        "height": $(window).height() - (self.tCon.height() + self.lCon.height() + 30)
                    });
                    self.pHol.css({
                        "position": "relative",
                        "top": "0px",
            ...
            setTimeout(function () {
                $(".pageHolder .page").find(".fs_gallery").each(function () {
                    if (view) $(this).css({
                        "height": $(this).children(":first-child").children(":last-child").position().top + self.fsThuHig + 15
                    })
                });
                self.oScrollbar.tinyscrollbar_update(0)
            }, 400)
        ...
        bg_setup: function () {
            ...
            $(".pageHolder .page").find("#mapWrapper").each(function () {
                if (!self.IE_old) $(this).children(":first-child").remove()
            });
            $(".pageHolder .page").find(".addVideo").each(function () {
                $(this).find("#vid").remove();
                $(this).find("img").show()
            ...
            for (var ii = 0; ii < self.menuList.length; ii++) {
                ...
                if (gg[1] == self.curPg) imgSrc = String(self.pageBackground[ii].src)
            }
            if (imgSrc != "") {
                var _im = $("<img />");
                _im.hide();
        ...
        bg_load: function () {
            ...
            if (!self.pageLoaded) {
                ...
                self.tCon.fadeIn(1E3, function () {
                    self.higLig.css("display", "block");
                    self.higLig.animate({
                        "width": self.cM_.width(),
                        "left": self.cM.position().left + parseInt(self.cM_.css("padding-left")),
                        "top": self.cM.position().top
                    })
                });
                self.foot.fadeIn(1E3);
                $(".logo").css("visibility", "visible");
                $(".logo").hide();
                $(".logo").fadeIn();
                if (!self.IE_old) $(".portfolio .item a .img_text").css("visibility", "visible")
            }
            self.loading.fadeOut(300, function () {
                $(this).css("visibility", "hidden")
            });
        ...
        page_setup: function () {
            ...
            if (!self.mobile || self.IE_old) {
                ...
                if (isNaN(self.pgHeight)) self.pHol.css({
            ...
            self.oScrollbar.tinyscrollbar_update();
            if (self.curPg == "") self.pHol.css("left", ($(window).width() - self.pHol.width()) / 2 + (self.pHol.width() + 50) + "px");
            self.bdy.find("img").each(function () {
                if (!self.mobile || self.IE_old || !$(this).attr("data-src-small")) {
                    if ($(this).data("src")) if ($(this).data("src") != $(this).attr("src")) $(this).attr("src", $(this).data("src"))
                } else if ($(this).attr("data-src-small")) if (String($(this).attr("src")) != String($(this).attr("data-src-small"))) $(this).attr("src", $(this).attr("data-src-small"))
            });
            $(".pageHolder .page").find(".addVideo").each(function () {
            ...
            self.fsNxt.css({
                "top": $(window).height() / 2 + self.fsNxt.height() / 2 + "px"
            });
            self.fsPre.css({
                "top": $(window).height() / 2 + self.fsPre.height() / 2 + "px"
            });
            self.fsTxt.css({
            ...
            $(".pageHolder .page").find(".fs_gallery").each(function () {
                if (self.mobile || self.mobileDevice || self.fsTxt.data("pg") != $(".pageHolder .page").attr("data-id")) {
                    if (self.fsTxt.children().length > 0) self.fsTextOut()
                } else if (self.fsTxt.children().length < 1) self.fsTextIn();
                if (!self.mobile) {
                ...
                if (parseInt(self.pHol.css("left")) == 0) if (self.fsView) self.fsGallHide(false);
                else self.fsGallShow(false)
            })
        },
        page_load: function (e) {
            ...
            if (self.prePg == self.curPg) {
                ...
                if (self.curPg == "") {
                    ...
                    if (self.curPg == self.homePg) self.bg_setup();
                    else self.bg_setup()
                } else {
                    self.curPg = self.url;
                    self.pHol.stop();
                    ...
                    clearInterval(self.fsImgInt);
                    self.fsTxtSho = false;
                    if (self.fsTxt.children().length > 0) if (!self.mobileDevice) self.fsTextOut();
                    self.isFsGal = false;
                    $(".pageHolder .page").find(".fs_gallery").each(function () {
                        self.isFsGal = true
                    });
                    if (self.isFsGal) if (parseInt(self.pHol.css("height")) == self.pgHeight) self.pHol.animate({
                        "left": -(self.pHol.width() + 50) - ($(window).width() - self.pHol.width()) / 2
                    }, 1E3, "easeInOutQuart", function () {
                        self.bg_setup()
                    });
                    else self.pHol.animate({
                        "height": 0
                    }, 1E3, "easeInOutQuart", function () {
                        self.pHol.css({
                            "left": -(self.pHol.width() + 50) - ($(window).width() - self.pHol.width()) / 2
                    ...
                    else if (self.prePg != self.url) self.pHol.animate({
                        "left": -(self.pHol.width() + 50) - ($(window).width() - self.pHol.width()) / 2
                    }, 1E3, "easeInOutQuart", function () {
                        self.bg_setup()
                    })
            ...
            $(".headerContent li a").css({
                "color": self.menuColor
            });
            $("a", self.cM).css({
                "color": self.menuHighlightColor
            });
            if (!self.mobile) {
                self.higLig.show();
                self.higLig.animate({
        ...
        page_display: function () {
            ...
            if (self.curPg != "") {
                ...
                if (!self.IE_old) $(".pageHolder .page").find(".portfolio").each(function () {
                    var gaIt = $(".portfolio .item");
                    if (self.midMobile) {
                        gaIt.removeClass("large");
                        gaIt.removeClass("small");
                ...
                } else self.oScrollbar.tinyscrollbar_update();
                $.vegas("resize_");
                self.page_setup();
                ...
                ppg.find("ul.feature_wrap li .fea").css({
                    "height": "0%",
                    "opacity": "0"
                });
                self.fsCur = -1;
                ...
                ppg.find(".fs_gallery").each(function () {
                    ...
                    $(this).find(".fs_thumbs").each(function () {
                        $(this).children().each(function () {
                            ...
                            if ($(this).css("opacity") < 0.99) $(this).css({
                                "opacity": "1"
                            });
                ...
                function () {
                    ...
                    if (self.curPg != self.url) self.page_load(self.url);
                    ppg = $(".pageHolder .page");
                    ppg.find("#mapWrapper").each(function () {
                        if (!self.IE_old) $(this).append($(this).data("map"))
                    });
                    ppg.find(".portfolio").each(function () {
                        if (!$(this).data("loaded")) $(this).isotope({
                            itemSelector: ".item",
                            animationOptions: {
                                complete: function () {
                                    self.oScrollbar.tinyscrollbar_update()
                    ...
                    var iu = 0;
                    if (self.IE_old) ppg.find("ul.feature_wrap li").show();
                    ppg.find("ul.feature_wrap li .fea").each(function () {
                        iu++;
                        $(this).parent().show();
                        $(this).delay(Number(iu * 150)).animate({
                            "height": "100%",
                            "opacity": "1"
                        }, 1200, "easeInOutQuart")
                    });
        ...
        history: function () {
            ...
            (function ($) {
                ...
                function loadContent(hash) {
                    if (hash != "") {
                        if (origContent == "") origContent = $(self.homePage);
                        if (self.hisPath != hash) {
                            self.hisPath = hash;
                            self.page_load(hash)
                        }
                    } else if (origContent != "") if (self.hisPath != hash) {
                        self.hisPath = hash;
                        self.page_load(self.homePg)
                    }
                    if (hash == "") self.page_load(self.homePg)
                }
    ...
    $.fn.mainFm = function (params) {
        ...
        if (!instance) {
            if (typeof params === "object" || !params) return $fm.data("GBInstance", new mainFm($fm, params))
        } else if (instance[params]) return instance[params].apply(this, Array.prototype.slice.call(arguments, 1))
    }
})(jQuery);